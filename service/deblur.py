...
def deblur_service(
    ...
    result_name = f'{task_current}_{generate_mode}_{model_name}_sigma{noise_level_img}_NFE{iter_num}_eta{eta}_zeta{zeta}_lambda{lambda_}_blurmode{blur_mode}'
    ...
    def test_rho(lambda_=lambda_, zeta=zeta, model_output_type=model_output_type):
        ...
        if use_DIY_kernel:
            np.random.seed(seed=10)  # for reproducibility of blur kernel for each image
                        ...
                        if sub_1_analytic:
                            if model_out_type == 'pred_xstart':
                                ...
                                if i < num_train_timesteps - noise_model_t:
                                    x0_p = x0 / 2 + 0.5
                                    if device.type == 'mps':
                                        x0_p = sr.data_solution(x0_p.float().to(torch.device('cpu')), FB, FBC, F2B,
                                                                FBFy, tau.to(torch.device('cpu')), sf).to(device)
                                    else:
        ...
        if save_LEH:
            ...
            leh_path = os.path.join(E_path, img_name + '_LEH' + ext)
            util.imsave(np.concatenate([img_I, img_E, img_H], axis=1), leh_path)
            path_to_return['leh'] = leh_path

        if save_L:
            low_path = os.path.join(E_path, img_name + '_LR' + ext)
            util.imsave(img_L, low_path)
