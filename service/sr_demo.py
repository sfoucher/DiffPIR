...
def sr_service_demo(
    ...
    result_name = f'{task_current}_{generate_mode}_{sr_mode}{str(test_sf)}_{model_name}_sigma{noise_level_img}_NFE{iter_num}_eta{eta}_zeta{zeta}_lambda{lambda_}'
                ...
                if calc_LPIPS:
                    ...
                    logger.info(
                        '------> Average LPIPS of ({}) scale factor: ({}), kernel: ({}) sigma: ({:.3f}): {:.4f}'.format(
                            input_image_path, sf, k_index, noise_level_model, ave_lpips_k))
